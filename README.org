...
LKDE is very simple and easy to hack with compared to similar
projects, making it both powerful and fun.
...
You can get a list of the most useful settings and their values with
=make settings=, or simply by reading the root makefile:
...
First, you should build your tools for the target architecture, like
gcc, binutils and qemu.  You can run =make deps= do download and
compile everything. The sources will be placed in =deps/= and
installed in =usr/= or =usr/$TARGET_ARCH=:
...
To run the kernel, you need an a userspace filesystem, which you can
setup with:
